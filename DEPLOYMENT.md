...
3. Get your two connection strings from Project Settings → Database → Connection string:
   - **Transaction pooler** (port 6543) → this becomes `DATABASE_URL`. Append `?pgbouncer=true&connection_limit=5` to the end — **not** `connection_limit=1`, see the comment in `server/prisma/schema.prisma`'s datasource block for why that specific value was the confirmed cause of slow page loads.
