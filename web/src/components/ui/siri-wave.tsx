...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    float inten = mix(0.1, 0.01*(INTENSITY + low*LOW_INT) * uBright, res);
...
vec3 scene(vec2 p, float t){
    ...
    float gBright = mix(1.0, GATHER_DIM, gC) * (1.0 + CHARGE_GLOW*charge + FLASH_GAIN*flash) * uBright;
...
export interface SiriWaveProps extends Omit<React.HTMLAttributes<HTMLCanvasElement>, "children"> {
  ...
  size?: number;
  renderScale?: number;
  /** Speeds up + brightens the animation — use for a "thinking" state. */
  active?: boolean;
}
...
export function SiriWave({
  ...
  React.useEffect(() => {
    ...
    const frame = () => {
      ...
      lastNow = now;
      const speed = activeRef.current ? 1.6 : 0.65;
      simTime += dt * speed;
      gl.uniform2f(uResolution, dim, dim);
      gl.uniform1f(uTime, simTime);
      gl.uniform1f(uBright, activeRef.current ? 1.35 : 0.85);
  ...
  return (
    <canvas
      ref={canvasRef}
      className={cn("block rounded-full bg-ink-900", className)}
